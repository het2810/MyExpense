...
function createStyles(theme, insets) {
  return StyleSheet.create({
    glowWrapper: {
      ...
      // Both offsets above are stated for a device with no bottom system
      // bar. `getFabBottomOffset` folds in the same `insets.bottom` that
      // `getTabBarBottomOffset` does, so the 16px gap between the two
      // overlays holds on every device — they move together, never into
      // each other.
      bottom: getFabBottomOffset(theme, insets),
